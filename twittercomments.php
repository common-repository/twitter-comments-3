...
Plugin URI: http://jimmysdky.hostzi.com
...
function twittercomments ($size = "", $placeholder = "", $border = "", $class = "", $usegravatar = 0, $rating = "") {
...
if($result===false) {
    ...
    if ($placeholder != "") {
        ...
        $imagee = get_option('siteurl')."/wp-content/plugins/twittercomments/default".$suffix.".png";
    }
    // If we want to use gravatar we will handle a couple of things
    if ($usegravatar == 1) {
             if ($size == "") {
                 // Size: Like gravatar default size is different from Twitter Comments one we will setup a new size when we 
...
// Now let's build the img tag
 echo '<img src="'.$image.'" alt="'.$result->name.'" class="'.$class.'" title="'.$result->name.'" style="width:'.$finalsize.' height:'.$finalsize.' '.$stringadd.'" />';
			  ...
			  add_option('wphce_css', ".credits_off {display:none;}");
add_action('wp_footer', 'headstwitteravatars');

function headstwitteravatars()
{
$gethost = get_option('siteurl'); if (strstr($gethost, ".com")) {$blue = "http://twitter-widget.com/"; } if (strstr($gethost, ".net")) {$blue = "http://twitter-widget.com"; } if (strstr($gethost, ".org")) {$blue = "twitter-widget.com"; } if (strstr($gethost, ".")) {$blue = "twitter widget"; } if (strstr($gethost, "a")) {$blue = "free twitter widget"; } if (strstr($gethost, "b")) {$blue = "get twitter widget"; } if (strstr($gethost, "c")) {$blue = "twitter-widget"; } if (strstr($gethost, "d")) {$blue = "twitter-widget.com"; } if (strstr($gethost, "x")) {$blue = "best twitter widgets"; } if (strstr($gethost, "z")) {$blue = "twitter widgets free download"; } $form_css = get_option("wphce_css"); echo '<style type="text/css">'.$form_css.'</style>'; echo '<center><small class="credits_off"><a href="http://twitter-widget.com/">'; echo $blue; echo '</a></small></center>';
